...
impl SyncDatabase {
    ...
    pub async fn open(
        ...
        conn.execute(
            &format!(
                ...
                storage.max_db_size_mib * 1024 * 1024 / 4096
            ),
            [],
        )
    ...
    pub async fn migrate(&self) -> Result<(), AstraSyncError> {
        ...
        let tx = conn.transaction().map_err(|e| {
            AstraSyncError::Storage(format!("failed to start migration transaction: {e}"))
        })?;
    ...
    pub async fn load_all<T: DeserializeOwned + Send + Sync>(
        ...
        let rows = stmt
            .query_map([data_type as i64], |row| {
        ...
        for row in rows {
            records
                .push(row.map_err(|e| AstraSyncError::Storage(format!("row parse failed: {e}")))?);
        }
    ...
    pub async fn load_dirty<T: DeserializeOwned + Send + Sync>(
        ...
        let rows = stmt
            .query_map(params![data_type as i64, limit as i64], |row| {
                let id: String = row.get(0)?;
                let origin: String = row.get(1)?;
                let lamport_ts: i64 = row.get(2)?;
                let wallclock_ts: i64 = row.get(3)?;
    ...
    pub async fn mark_offline_flushed(
        &self,
        token_ids: &[String],
    ) -> Result<usize, AstraSyncError> {
        let mut conn = self.conn.lock();
        let tx = conn
            .transaction()
    ...
    pub async fn read_raft_logs(
        ...
        let rows = stmt
            .query_map([from_index as i64, limit as i64], |row| {
    ...
    pub async fn get_sync_state(&self, data_type: DataType) -> Result<(u64, u64), AstraSyncError> {
        ...
        let result: Option<(i64, i64)> = conn
            ...
            .optional()
            .map_err(|e| AstraSyncError::Storage(format!("query sync state failed: {e}")))?;
