 ...
 * CRDT merge worker.
 *
 * Heavy CRDT merge operations (cart/inventory conflict resolution) and
 * cryptographic hashing (merkle-chain event hashes) run in a dedicated
 * Web Worker backed by a Rust-compiled WASM module (`astra_crdt_wasm`,
 * built from daemons/astra-syncd's CRDT crate via wasm-pack — see
 * daemons/astra-syncd/README.md "Browser WASM target"). This keeps the
 * main thread free for 60fps touch input handling even while merging a
 * large offline backlog after a 48h network outage.
 */
...
async function ensureWasmLoaded(): Promise<CrdtWasmModule> {
  ...
  // Dynamic import so the ~200KB WASM binary is only fetched once the first
  // cart mutation actually needs merging, not on kiosk cold boot.
